...
import { CreateIParams, IQuery, IServiceInput, ISessionDataExt } from "../../base/IBase";
import { CdObjTypeModel } from "../../moduleman/models/cd-obj-type.model";
import { GroupModel } from "../../user/models/group.model";
import { IUserProfile, profileDefaultConfig, UserModel, userProfileDefault } from "../../user/models/user.model";
import { SessionService } from "../../user/services/session.service";
import { UserService } from "../../user/services/user.service";
import { CdCliProfileModel, ICdCliProfileProfile, IUserProfileOnly } from "../models/cd-cli-profile.model";
...
export class CdCliProfileService extends CdService {
    ...
    async create(req, res) {
        ...
        this.b.i.app_msg = 'CdCli members processed';
    ...
    async validateCreate(req, res) {
        ...
        console.log("CdCliProfileService::validateCreate/this.b.err1:", JSON.stringify(this.b.err));

        if (!valid) {
            this.logger.logInfo('cdCli/CdCliProfileService::validateCreate()/Validation failed');
    ...
    async validateExistence(req, res, validationParams) {
        const promises = validationParams.map(param => {
            ...
            console.log("CdCliProfileService::validateExistence/param.model:", param.model);
            console.log("CdCliProfileService::validateExistence/serviceInput:", JSON.stringify(serviceInput));
            const b = new BaseService();
            return b.read(req, res, serviceInput).then(r => {
                if (r.length > 0) {
                    this.logger.logInfo(`cdCli/CdCliProfileService::validateExistence() - ${param.field} exists`);
                    return true;
                } else {
                    this.logger.logError(`cdCli/CdCliProfileService::validateExistence() - Invalid ${param.field}`);
                    this.b.i.app_msg = `${param.field} reference is invalid`;
                    this.b.err.push(this.b.i.app_msg);
                    console.log("CdCliProfileService::validateExistence/this.b.err1:", JSON.stringify(this.b.err))
                    return false;
                }
            });
        });
        ...
        console.log("CdCliProfileService::validateExistence/this.b.err2:", JSON.stringify(this.b.err))
    ...
    async createI(req, res, createIParams: CreateIParams): Promise<CdCliProfileModel | boolean> {
        // const svSess = new SessionService()
    ...
    async validateUpdateCdCliProfileProfile(req, res) {
        ...
        if (req.post.a !== 'UpdateCdCliProfileProfile' || !('userId' in plQuery.where)) {
            ret = false
        }
        return ret
    }
    ...
    async getCdCliProfileI(req, res, q: IQuery = null): Promise<CdCliProfileViewModel[]> {
        if (q === null) {
            q = this.b.getQuery(req);
        }
    ...
    async getCdCliProfileCount(req, res) {
        ...
        console.log('CdCliProfileService::getCdCliProfileCount/q:', q);
        const serviceInput = {
            serviceModel: CdCliProfileViewModel,
    ...
    async validateProfileData(req, res, profileData: any): Promise<boolean> {
        ...
        if (!profileData.fieldPermissions || !profileData.userData) {
            ...
            console.log("CdCliProfileService::validateProfileData()/profileData.userData:", profileData.userData)
            console.log("CdCliProfileService::validateProfileData()/profileData.fieldPermissions:", profileData.fieldPermissions)
            return false;
        }
